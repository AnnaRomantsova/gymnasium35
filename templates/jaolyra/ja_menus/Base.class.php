...
if (!defined ('_JA_BASE_MENU_CLASS')) {
        ...
        class JA_Base{
                ...
                function JA_Base( &$params ){
                        ...
                        $active = $m->getActive();
    					$this->Itemid = isset($active) ? $active->id : $m->getDefault()->id;
            ...
            function  loadMenu_15(){
                ...
                $user =& JFactory::getUser();
                       ...
                       $rows = $items->getItems('menutype', $this->getParam('menutype'));
                        //var_dump($items->getItems('menutype', $this->getParam('menutype')));
                         //echo $this->getParam('menutype');
		                // first pass - collect children
		               $cacheIndex = array();
                       $this->items = array();
                       $aid = 1;//$user->get('gid', 0);   //было    $aid = $user->get('aid', 0);
                      if (is_array($rows)){
                ...
                function genMenuItem($item, $level = 0, $pos = '', $ret = 0)
                {
                        ...
                        if ($this->getParam('menu_images') && $iParams->get('menu_image') && $iParams->get('menu_image') != -1) {
                                if ($this->getParam('menu_background')) {
                                        ...
                                        $txt = '<span>' . $tmp->title . '</span>';
                                } else {
                                        $txt = '<img src="images/stories/'.$iParams->get('menu_image').'" alt="'.$tmp->title.'" title="'.$tmp->title.'" /><span>' . $tmp->name . '</span>';
                                }
                        } else {
                                $txt = '<span>' . $tmp->title . '</span>';
                        }
